...
ObjectSetText("label_object4","Вторая серия Sell="+SecondSellSeries+"; Третья серия Sell="+ThirdSellSeries,12,"Arial",Red);
   ...
   ReCountBuy=0;ReCountSell=0;ReBuyLots=0;ReSellLots=0;bb=0;ss=0;
   //SellGoToZero=false;BuyGoToZero=false;
...
//Локирование ордеров 
//if ((BuyGoToZero==true)&&(ReCountSell!=0)&&(CountSell==(ReCountSell-1))){StopSell();}
//if ((SellGoToZero==true)&&(ReCountBuy!=0)&&(CountBuy==(ReCountBuy-1))){StopBuy();}
...
if (ReCountSell>1) {SearchFirstBuyOrder(); SearchLokSellOrdersProfit(); OrderSelect(Ticket, SELECT_BY_TICKET);FirstBuyOrderProfit=OrderProfit();if(FirstBuyOrderProfit!=0){
  ...
  if(OrderLots()/4*CoeffNull<ReBuyLots){
  if((SellOrdersProfit+FirstBuyOrderProfit/4)>BonusDollar*100){Print("Закрываем 1/4 первого ордера на покупку и ордера на продажу");CloseQuatFirstBuySellOrders();}}
  
 // else{if((SellOrdersProfit+FirstBuyOrderProfit/8)>BonusDollar*100){Print("Закрываем 1/8 первого ордера на покупку и ордера на продажу");CloseQuatQuatFirstBuySellOrders();}}
...
if (ReCountBuy>1) {SearchFirstSellOrder();SearchLokBuyOrdersProfit();OrderSelect(Ticket, SELECT_BY_TICKET);FirstSellOrderProfit=OrderProfit();if(FirstSellOrderProfit!=0){
  ...
  else {  if(OrderLots()/4*CoeffNull<ReBuyLots) 
  {if((BuyOrdersProfit+FirstSellOrderProfit/4)>BonusDollar*100){Print("Закрываем 1/4 первого ордера на продажу и ордера на покупку");CloseQuatFirstSellBuyOrders();}}
  else {if((BuyOrdersProfit+FirstSellOrderProfit/8)>BonusDollar*100){Print("Закрываем 1/8 первого ордера на продажу и ордера на покупку");CloseQuatQuatFirstSellBuyOrders();}}
...
double SearchSecondSellOrder() {
 ...
 double CloseFirstBuySellOrders()
   ...
   double CloseQuatFirstBuySellOrders()
  {
   ...
   double CloseQuatQuatFirstBuySellOrders()
  {
   SearchFirstBuyOrder();
         OrderSelect(Ticket, SELECT_BY_TICKET); double j=OrderLots()/8;  OrderClose(Ticket,j,Bid,3*k,Black);
    ...
    for(int SS=OrdersTotal();SS>0;SS--)
     {
      ...
      return(0);
       ...
       double CloseQuatQuatFirstSellBuyOrders()
  {
        SearchFirstSellOrder();
      OrderSelect(Ticket, SELECT_BY_TICKET); double t=OrderLots()/8;
      OrderClose(Ticket,t,Ask,3*k,Black);
    for(int SS=OrdersTotal();SS>0;SS--)
     {
      if(OrderSelect(SS,SELECT_BY_POS)==true)
   ...
   if(CountB>0)
     {
        ...
        SearchFirstSellOrder();
           OrderSelect(Ticket, SELECT_BY_TICKET);
       TPB=((PriceB+OrderOpenPrice()*(-OrderLots()))/(BuyLots+(-OrderLots())))+TP2*k*Point;
     
       
       OrderModify(OrderTicket(),OrderOpenPrice(),TPB+Spread*k*Point,OrderTakeProfit(),0,Orange); 
      for(int ibuy3Result=0;ibuy3Result<OrdersTotal();ibuy3Result++)
        { 
         if(OrderSelect(ibuy3Result,SELECT_BY_POS)==true)
           {
