...
ObjectSetText("label_object3","Текущая просадка="+DoubleToStr(TotalProfit,2),12,"Arial",Red);
 ...
 if((ReCountBuy+Index)<OrdersToZero){BuyGoToZero=false;kLevelSell=1;}
 if((ReCountSell+Index)<OrdersToZero){SellGoToZero=false;kLevelBuy=1;}
...
if (((BuyLok==true)||(SellLok==true))&&(TotalProfit>(BonusDollar*100))&&(ReCountSell!=0)&&(ReCountBuy!=0)){Print("Закрываем все ордера");CloseAllOrders();BuyLok=false;SellLok=false;}
               ...
               if(( OrderSymbol()==Symbol()) && (OrderType()==OP_BUY)) 
                 {
                  ...
                  Lot=NormalizeDouble(OrderLots()*CoefLot,2);  if (Lot>CritLot){Lot=CritLot;}
                 }
                 else {Lot=NormalizeDouble(OrderLots()*CoefLot2,2); if (Lot>CritLot){Lot=CritLot;}}
                 
                 }               
              }
           }
         if(Ask<(LastBuyPrice-Level1*kLevelBuy*k*Point))
           {
      ...
      if((CountSell==1) && (SellTrade==true)&&(BuyGoToZero==false)&&(SellGoToZero==false))
        {
         ...
         if(Bid>(LastSellPrice+Level1*k*kLevelSell*Point))
           {
      ...
      if((CountBuy==2) && (BuyTrade==true)&&(SellGoToZero==false)&&(BuyGoToZero==false))
        {
         ...
         if(Ask<(LastBuyPrice-Level2*kLevelBuy*k*Point))
           {
        
            Print("Открываем 3-й ордер Buy");
             SearchLastBuyLot();      
              if (SellGoToZero==false){
                  Lot=NormalizeDouble(LastBuyLot*CoefLot,2);  if (Lot>CritLot){Lot=CritLot;}
                 }
                 else {Lot=NormalizeDouble(LastBuyLot*CoefLot2,2); if (Lot>CritLot){Lot=CritLot;}}
      ...
      if((CountSell==2) && (SellTrade==true)&&(BuyGoToZero==false)&&(SellGoToZero==false))
        {
         ...
         if(Bid>(LastSellPrice+Level2*kLevelSell*k*Point))
           {
         
            Print("Открываем 3-й ордер Sell");
            SearchLastSellLot();    
            if (BuyGoToZero==false){
                 Lot=NormalizeDouble(LastSellLot*CoefLot,2);  if (Lot>CritLot){Lot=CritLot;}
                 }
                 else {Lot=NormalizeDouble(LastSellLot*CoefLot2,2); if (Lot>CritLot){Lot=CritLot;}}
            if(IsTradeAllowed()) 
              {
      ...
      if((CountBuy==3) && (BuyTrade==true)&&(SellGoToZero==false)&&(BuyGoToZero==false))
        {
         ...
         if(Ask<(LastBuyPrice-Level3*kLevelBuy*k*Point))
           {
           
            Print("Открываем 4-й ордер Buy");
                    SearchLastBuyLot();       if (SellGoToZero==false){
                  Lot=NormalizeDouble(LastBuyLot*CoefLot,2);  if (Lot>CritLot){Lot=CritLot;}
                 }
                 else {Lot=NormalizeDouble(LastBuyLot*CoefLot2,2); if (Lot>CritLot){Lot=CritLot;}}
            if(IsTradeAllowed()) 
      ...
      if((CountSell==3) && (SellTrade==true)&&(BuyGoToZero==false)&&(SellGoToZero==false))
        {
         ...
         if(Bid>(LastSellPrice+Level3*kLevelSell*k*Point))
           {
            
            Print("Открываем 4-й ордер Sell");
            SearchLastSellLot();    if (BuyGoToZero==false){
                  Lot=NormalizeDouble(LastSellLot*CoefLot,2);  if (Lot>CritLot){Lot=CritLot;}
                 }
                 else {Lot=NormalizeDouble(LastSellLot*CoefLot2,2); if (Lot>CritLot){Lot=CritLot;}}
            if(IsTradeAllowed()) 
      ...
      if(((CountBuy==6)||(BComment=="16")) && (BuyTrade==true)&&(SellGoToZero==false))
        {
         ...
         if(Ask<(LastBuyPrice-Level6*kLevelBuy*k*Point))
           {
      ...
      if(((CountSell==6)||(SComment=="26")) && (SellTrade==true)&&(BuyGoToZero==false))
        {
         ...
         if(Bid>(LastSellPrice+Level6*kLevelSell*k*Point))
           {
  ...
  for (int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      if ((OrderType()==OP_SELLSTOP )&&(OrderMagicNumber() == Magic_Number )) if (IsTradeAllowed()){if (OrderDelete(OrderTicket())<0)
       ...
       if (Hour()==0){if (((OrderType()==OP_SELLSTOP )||(OrderType()==OP_BUYSTOP ))&&(OrderMagicNumber() == Magic_Number )){if (IsTradeAllowed()){if (OrderDelete(OrderTicket())<0)
           { 
        Alert("Ошибка удаления ордера № ", GetLastError()); 
      } 
      }}
...
//#Открытие первого ордера buy
   if((ReCountBuy==0)&&(SellGoToZero==false) && (BuyTrade==true)&&((((iMA(NULL,0,200,0,MODE_SMA,PRICE_CLOSE,1)+50*Point*k)<Close[1])&&(iATR(NULL,0,22,1)>0.0002)&&(iWPR(NULL,0,21,1)<-95)&&(iCCI(Symbol(),0,15,PRICE_TYPICAL,1)<-95))||
   (((Minute()==15)||(Minute()==30)||(Minute()==45)||(Minute()==00))&&((iMA(NULL,15,110,0,MODE_SMA,PRICE_CLOSE,1)+5*Point*k)<Close[1])&&(iATR(NULL,15,22,1)>0.0002)&&(iWPR(NULL,15,22,1)<-95)&&(iCCI(Symbol(),15,16,PRICE_TYPICAL,1)<-95))||
   (((Minute()==30)||(Minute()==00))&&((iMA(NULL,30,180,0,MODE_SMA,PRICE_CLOSE,1)+50*Point*k)<Close[1])&&(iATR(NULL,30,22,1)>0.0002)&&(iWPR(NULL,30,21,1)<-95)&&(iCCI(Symbol(),30,15,PRICE_TYPICAL,1)<-95))||
   (((Minute()==00))&&((iMA(NULL,60,200,0,MODE_SMA,PRICE_CLOSE,1)+50*Point*k)<Close[1])&&(iATR(NULL,60,22,1)>0.0002)&&(iWPR(NULL,60,21,1)<-95)&&(iCCI(Symbol(),60,15,PRICE_TYPICAL,1)<-95))
...
if (SellGoToZero==false){
      if(IsTradeAllowed()) 
        {
if (IsTradeAllowed()) { if(    OrderSend(Symbol(),OP_BUY,Lot1*GoGoBuy,Ask,3*k,NULL,Ask+TP*k*Point,"11",Magic_Number,0,Blue) < 0) 
      {Alert("Ошибка открытия позиции № ", GetLastError()); }}
      }
       } 
...
//#Открытие первого ордера sell
   if((ReCountSell==0)&&(BuyGoToZero==false) && (SellTrade==true)&&((((iMA(NULL,0,200,0,MODE_SMA,PRICE_CLOSE,1)-50*Point*k)>Close[1])&&(iATR(NULL,0,22,1)<0.0002)&&(iWPR(NULL,0,21,1)>-5)&&(iCCI(Symbol(),0,15,PRICE_TYPICAL,1)>90))||
   (((Minute()==15)||(Minute()==30)||(Minute()==45)||(Minute()==00))&&((iMA(NULL,15,110,0,MODE_SMA,PRICE_CLOSE,1)+5*Point*k)>Close[1])&&(iATR(NULL,15,22,1)<0.0002)&&(iWPR(NULL,15,22,1)>-5)&&(iCCI(Symbol(),15,16,PRICE_TYPICAL,1)>90))||
   (((Minute()==30)||(Minute()==00))&&((iMA(NULL,30,180,0,MODE_SMA,PRICE_CLOSE,1)+50*Point*k)>Close[1])&&(iATR(NULL,30,22,1)<0.0002)&&(iWPR(NULL,30,21,1)>-5)&&(iCCI(Symbol(),30,15,PRICE_TYPICAL,1)>90))||
   (((Minute()==00))&&((iMA(NULL,60,200,0,MODE_SMA,PRICE_CLOSE,1)+50*Point*k)>Close[1])&&(iATR(NULL,60,22,1)<0.0002)&&(iWPR(NULL,60,21,1)>-5)&&(iCCI(Symbol(),60,15,PRICE_TYPICAL,1)>90))
    ...
    { 
     ...
     if(BuyGoToZero==true){  GoGoSell=BuyLots*Percent/100/Lot1;NormalizeDouble(GoGoSell,2); } 
      ...
      if(IsTradeAllowed()) 
        {
         if(OrderSend(Symbol(),OP_SELLSTOP,Lot1*GoGoSell,Bid,3*k,NULL,Bid-TP*k*Point,"21",Magic_Number,0,Red) < 0)
           {Alert("Произошла ошибка",GetLastError()); }
        }
     }
